...
function P_exact(x::RealVector, Re::Float64, t::Float64)::Float64
    return v_max(Re, t)*(P0 + 0.5*sin(pi*x[1])^2 + 0.5*sin(pi*x[2])^2)
end
...
function step!(sim::Simulation, t::Float64)
    ...
    ideal_eos!(sim.grid, gamma, 1e-2*P0)
...
function test()
    sim = Simulation(100, Inf, true)
    path = joinpath(export_path, "test")
    run!(sim, sim.dt, t_end, step!; 
            postproc! = postproc!,
            nframes = 100, 
            path = path,
            save_csv = false,
            vtp_vars = (:P, :v, :v_err, :P_err)
    )
end
