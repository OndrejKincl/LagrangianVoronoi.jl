...
function populate_circ!(grid::VoronoiGrid{T}, dr::Float64; charfun = (::RealVector -> true), center::RealVector = VEC0) where T
...
function populate_rect!(grid::VoronoiGrid{T}, dr::Float64; charfun = (::RealVector -> true)) where T
...
function populate_rand!(grid::VoronoiGrid{T}, dr::Float64; charfun = (::RealVector -> true)) where T
    x1_max = grid.boundary_rect.xmax[1]
    x2_max = grid.boundary_rect.xmax[2]
    x1_min = grid.boundary_rect.xmin[1]
    x2_min = grid.boundary_rect.xmin[2]
...
function populate_vogel!(grid::VoronoiGrid{T}, dr::Float64; charfun = (::RealVector -> true), center::RealVector = VEC0) where T
    r_max = maximum([norm(x - center) for x in verts(grid.boundary_rect)])
    N = round(Int, pi*r_max*r_max/(dr*dr))
    for i in 1:N
        r = r_max*sqrt(i/N)
